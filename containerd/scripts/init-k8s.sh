#!/usr/bin/env bash
# Initialize k8s cluster with containerd runtime
set -euo pipefail
...
kubeadm init --node-name="$(hostname)" --apiserver-advertise-address="${IP_ADDR}" --pod-network-cidr=192.168.0.0/16
...
# Node Ready doesn't guarantee service accounts created yet - give it 5s
sleep 5
