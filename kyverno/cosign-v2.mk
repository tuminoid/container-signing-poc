# setup kyverno test for e2e (cosign v2)

SCRIPTS_DIR := scripts
POLICY_DIR := policy

COSIGN_DIR := ../cosign-v2
...
test-pod:
	kubectl run --image $(CLUSTER_REGISTRY)/$(COSIGN_TEST_IMAGE_UNSIGNED) pod-fail-cosign-v2 || true
	kubectl run --image $(CLUSTER_REGISTRY)/$(COSIGN_TEST_IMAGE_SIGNED) pod-success-cosign-v2 || true

test-deployment:
	kubectl create deployment --image $(CLUSTER_REGISTRY)/$(COSIGN_TEST_IMAGE_UNSIGNED) deployment-fail-cosign-v2 || true
	kubectl create deployment --image $(CLUSTER_REGISTRY)/$(COSIGN_TEST_IMAGE_SIGNED) deployment-success-cosign-v2 || true
